<?
	...
	//LOG
	define('DAYS_COUNT', 30);
	function getLogs($path){
		$arLogs = array();

		if ($cortex = opendir($path)) {

			while (($file = readdir($cortex)) !== false) {
	...
	function writeToLog($data, $title = '') {
		...
		$arListFile = getLogs(__DIR__ . '/log/');
		$size       = count($arListFile);
		

		if($size > DAYS_COUNT) {
			array_splice($arListFile, $size - DAYS_COUNT);
		...
		$log .= print_r($data, 1);
		$log .= "\n------------------------\n";
		file_put_contents(__DIR__ . '/log/add_lead_' . date("Y_m_d") . '.log', $log, FILE_APPEND);
		return true;
	}	